...
序列破坏掉,**静默写坏文件**。两个路径(`tools/apply_patch.py` 与
`deploy-portal/tools/apply_patch.py`)背后是**同一份实现**:后者是本体,前者是
转发入口。补丁后端优先用本机跑得起来的 `codex`;没有可用 codex 时自动改用内置的
严格补丁引擎(CI 上就是这种情况,见 `.codex-specs/ci-portability/`)。显式写
`--backend codex` 却用不了时会直接报错,不会悄悄换后端。
...
补丁被拒(报「找不到上下文」)但那段原文肉眼可见时,先查这两条——它们都是
2026-09-17 实测出来的,不是猜的:

1. **同一补丁里的多个 `@@` 块,必须按「在文件里的位置」从前往后排。** 每一块都从
   上一块结束的位置往后找,顺序反了第二块就报找不到——哪怕原文确实在文件里。
   (实测:先改第 5 行再改第 2 行 → 被拒;反过来 → 通过。)
2. **要删的原文必须「按整行对齐」**:从某行行首开始、到某行行尾结束。从行中间开始的
   片段一定被拒,而 Python 的 `text.count(old) == 1` 仍然会说它存在——
   **用「子串存在」校验锚点是会骗过你的**。(实测:整行 → 通过;同一行里的一段
   → 被拒。)抓锚点时按行号切片(`lines[a:b]`),别用 `find` 出来的位置。
