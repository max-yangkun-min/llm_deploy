...
- **平台是通用的。** 推荐与部署方法面向任意同型号硬件,不绑定某一台机器。
  可公开核实的权威来源(厂商页、官方文档、官方支持矩阵)是通用方案的唯一依据;
  本工作区的现场记录只能以 `*-local` 状态存在,并写明「本工作区现场记录,
  不是通用方法」。见 `docs/ROADMAP.md` 的 R2。
...
CI 会跑语法检查、冒烟测试(135 项)、实测值核对、shell 语法与入库行尾、GPU 目录
自洽、昇腾官方支持矩阵留痕、改文件工具的三条链路(含内置补丁引擎)、根目录残留物,
并检查工作流骨架文件与
...
`smoke_test.py` 里有一整段昇腾/双生态反回归断言(该段 `check(...)` 调用点现 34 条)钉住这些差别。
...
- **不把现场资产提拔成通用方案**:不拿某台机器的离线包/验收记录当平台对某类卡的
  通用部署方法,也不把某台机器的具体配置(如「8×<某卡>」)写成对所有同生态卡的推荐。
  开源生态(`ecosystem: cann` 等)的方案必须来自官方文档/支持矩阵这类可公开核实、
  第三方能自己打开的来源。
