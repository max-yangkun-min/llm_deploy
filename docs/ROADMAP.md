...
| R3 | P1 | KV 上限反向提示(这张卡最多能开多长) | 无 | 已完成 2026-09-18 |
| R4 | P1 | `--kv-cache-dtype fp8` 作为输入项 | 无 | 已完成 2026-09-18 |
...
- 新增断言挂进 `smoke_test.py`,与现有 23 条昇腾/双生态反回归放在一起。
...
新增 `recommend.max_context_for(row, hw)`,是**唯一**的反算实现,`assess()` 直接调用它。
它把正向核算同一条 KV 线性公式倒过来:并行组可用显存
(`vram × PER_CARD_BUDGET × TP`)扣掉运行时余量与实测权重,剩下的 GiB 全给 KV,
再除以「1K token 的 KV 显存」。**复用同一批系数**,否则正算与反算会用两套假设。

`/api/recommend` 与 `/api/check` 的 `memory` 新增 5 个字段:`max_context_k`(上限)、
`max_context_note`(口径或为什么算不出)、`max_context_memory_k`(显存侧反算值)、
`max_context_model_k`(该档自身标称值)、`max_context_limit`(`memory` / `model`)。

四条分支都实测过:

| 分支 | 实测 |
|---|---|
| 显存先到顶 | 1×RTX 4090-24 上 `qwen3-coder-30b-awq` → 46.6K → 取 **46K**(`limit=memory`) |
| 标称上下文先到顶 | 8×A100-80 上 `glm52-int4-a100` → 显存侧 1091.9K、标称 1024K → 取 **1024K**(`limit=model`) |
| 权重本身就占满 | `deepseek-r1-bf16` 8×A100 → `null` +「权重(1275GiB)已经占满 8 张卡扣掉余量后的可用显存,没有任何空间留给 KV cache」 |
| KV 结构缺失 | `llama31-405b-bf16` → `null` +「attention 结构无法核实(config 取不到)」 |
| 昇腾 | 全部 `null` +「由 `--quantization ascend` 的专有量化决定,没有可核实的公开公式」 |

**超限时的判定**:填的上下文超过**显存反算值**时新增一条独立的失败信息,点明是 KV 超了
(「需求上下文 2000K 超出这张卡在此档的 KV 上限 1091K:该上下文要 171GiB 的 KV cache,
而扣除权重 442GiB 与 10% 运行时余量后只剩 94GiB 给 KV(这是 KV 超了,不是权重放不下)」)。

判据用 `max_context_memory_k` 而**不是** `max_context_k`:后者可能被「该档自身标称上下文」
压住(如 GLM 的 1024K),拿它当判据会把「其实显存够、是模型开不了那么长」说成「KV 超了」
——一句看着精确的错话。实测:8×A100 上 GLM 填 1050K 只留下「此档上下文 1024K 低于需求」,
不误报 KV;填 1200K 才两条都出。

界面:推荐页计划卡新增「最长上下文」一行(带「显存反算上限」/「受该档标称上下文限制」徽章,
算不出时「无法反算」+ 原因),Markdown 导出与台账达标检查表各加一列。

验收(实测):

- 自检 112 → **120** 项。关键的一条是**自洽性**:拿引擎自己给出的上限回填必须一条失败都没有,
  `+1K` 必须被判 KV 超。写这条时**第一次写错了**——只查「没有 KV 那条失败」,于是把反算公式
  乘 2(偏乐观)后 120 项**全绿**;改成「失败列表必须为空」后同一个改动立刻报
  `FAIL 引擎给出的上下文上限确实放得下 ['单卡需 27GiB(权重+KV+余量),超过 24GiB 卡的可用 22GiB']`。
  两个方向都验过:乘 2 → 该条 FAIL;除 2 → `FAIL 上下文上限是被标称上下文或显存卡住的`。
- 六视图浏览器复核 0 报错;证据 `output/playwright/25-ascend-max-context-null.png`、
  `26-max-context-cuda.png`。
...
**完成(2026-09-18)**。取值按官方口径收窄为 `auto` / `float16` / `bfloat16` / `fp8` /
`fp8_e4m3` / `fp8_e5m2`,默认 `auto`,未知取值当场 400(不静默回落)。判定分三档,
门槛依据是 vLLM 源码 `platforms/cuda.py` 的 `supports_fp8() = has_device_capability(89)`:

- 算力已核实且 < 8.9 → **判失败**。现场实测 A100(sm_80)与 A40(sm_86)开 fp8 KV 是
  启动即 `NotImplementedError`;只给警告然后照样按 1 字节算出「放得下」,等于推荐一个
  跑不起来的方案。说明里给出处置(保持 `auto`,或换 sm_89 及以上的卡)。
- 算力未登记 → 只给警告,并按 2 字节保守核算(不能假装知道,也不能替它判死刑)。
- 非 CUDA 生态(昇腾)→ **不判失败**,但必须写明不生效:KV 精度由 `--quantization ascend`
  的专有量化决定,`--kv-cache-dtype` 不是那套栈的参数。判失败会把「参数开不了」和
  「生态选错了」混成一件事。

后两种情形**都不减半**。R3 的反算与正算同口径:请求 fp8 时 `max_context_k` 一并按 1 字节
重算,不出现「正算按 1 字节、反算按 2 字节」的自相矛盾。

验收(实测):

- **对拍**:默认 `auto` 时与改动前逐字一致——8 个用例(含昇腾、`llama31-405b-bf16`
  的结构缺失分支)的 `memory`、`failures`、`warnings` 全部 **0 差异**。改默认值会把
  现有全部推荐结果静默改掉,所以这条是本次的回归基线。
- **减半**:RTX 4090(sm_89)+ `qwen3-coder-30b-awq`,KV 3.0 → 1.5 GiB,最长上下文
  46K → 93K;A100 的数字不变。
- **拦截**:8×A100 要 `fp8_e4m3` → 19 个方案全不进候选,失败信息带 `sm_89` 与
  `NotImplementedError`。
- **昇腾**:8×Atlas 300I Duo 要 `fp8_e4m3` → 不生效 + 说明,方案照常给出。
- 自检 120 → **135** 项(新增 15 条)。两个方向都反向验证过,都当场报红:
  把 fp8 的字节数改回 2 → 「真的减半」「上限变长」「低算力判失败」3 条 FAIL;
  让「不生效」分支仍按 1 字节返回 → 「不生效时数字必须是 2 字节口径」FAIL。
- 六视图浏览器复核 0 控制台报错。界面实测:4090 选 `fp8_e4m3` 显示「按 fp8_e4m3 生效 ·
  1 字节/元素」,A100 显示「未按 fp8_e4m3 生效(仍按 2 字节/元素)」+ 原因与处置,
